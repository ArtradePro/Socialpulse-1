    ...
    try {
        const { rows } = await db.query(
            `SELECT c.*,
              ...
              WHERE c.user_id = $1 AND (c.workspace_id = $2 OR ($2 IS NULL AND c.workspace_id IS NULL))
              GROUP BY c.id
...
export const getCampaign = async (req: Request, res: Response): Promise<void> => {
    ...
    const { rows: campaign } = await db.query(
        `SELECT * FROM campaigns WHERE id = $1 AND user_id = $2 AND (workspace_id = $3 OR ($3 IS NULL AND workspace_id IS NULL))`,
...
export const deleteCampaign = async (req: Request, res: Response): Promise<void> => {
    ...
    await db.query(`UPDATE posts SET campaign_id = NULL WHERE campaign_id = $1 AND user_id = $2 AND (workspace_id = $3 OR ($3 IS NULL AND workspace_id IS NULL))`,
        [id, req.user!.userId, req.workspaceId || null]);
    const { rowCount } = await db.query(
        `DELETE FROM campaigns WHERE id = $1 AND user_id = $2 AND (workspace_id = $3 OR ($3 IS NULL AND workspace_id IS NULL))`, 
        [id, req.user!.userId, req.workspaceId || null]
    );
