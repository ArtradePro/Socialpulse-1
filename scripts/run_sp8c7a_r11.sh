...
# GATE SP-8C-7A: REVISION R11 HOST SYNCHRONIZATION & PREFLIGHT EXECUTION WRAPPER
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A REVISION R11 HOST WRAPPER"
...
if [ "$(id -u)" -ne 0 ]; then
    echo "CRITICAL ERROR: run_sp8c7a_r11.sh must be executed as root (EUID 0)!" >&2
    exit 1
fi
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R11)
EXPECTED_SCRIPT_SHA256="9ee0a6014aff9c2eb70c1e899cbc9c9e33bc98919a57add1b193a9871a769138"
EXPECTED_SCRIPT_BYTES=50211
EXPECTED_SCRIPT_LINES=1207

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R11)
EXPECTED_SIDECAR_SHA256="903898f2c4c2bd213031a25405b528451e328f2929926e7b14cdc655bc7df52f"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R11)
EXPECTED_RUNBOOK_SHA256="dfe46201a7f1245eb2dbcede5f4af89ec0f534ea676fef15ec50879db42c1ede"
EXPECTED_RUNBOOK_BYTES=5003
EXPECTED_RUNBOOK_LINES=43

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R11)
EXPECTED_MANIFEST_SHA256="9527d35c6ed33149c284d317bcd2ed6884a89127f974b19ce6ea2d9a969cf282"
EXPECTED_MANIFEST_BYTES=2742
EXPECTED_MANIFEST_LINES=68
...
on_signal() {
    ...
    rollback_created_files || true
    cleanup_stage_dir || true
    cleanup_exec_out || true
    exit "${sig_exit}"
}
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 50211 "9ee0a6014aff9c2eb70c1e899cbc9c9e33bc98919a57add1b193a9871a769138"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "903898f2c4c2bd213031a25405b528451e328f2929926e7b14cdc655bc7df52f"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 5003 "dfe46201a7f1245eb2dbcede5f4af89ec0f534ea676fef15ec50879db42c1ede"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 2742 "9527d35c6ed33149c284d317bcd2ed6884a89127f974b19ce6ea2d9a969cf282"
...
assert rev == "R11", f"revision expected R11, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 50211,
        "lines": 1207,
        "sha256": "9ee0a6014aff9c2eb70c1e899cbc9c9e33bc98919a57add1b193a9871a769138",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "903898f2c4c2bd213031a25405b528451e328f2929926e7b14cdc655bc7df52f",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 5003,
        "lines": 43,
        "sha256": "dfe46201a7f1245eb2dbcede5f4af89ec0f534ea676fef15ec50879db42c1ede",
        "role": "operational_preflight_runbook_and_invariants_revision_r11"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R11, exactly 3 payload members with exact field values."
...
check_and_install_artifact "sp8c7a_preflight.sh" 50211 "9ee0a6014aff9c2eb70c1e899cbc9c9e33bc98919a57add1b193a9871a769138" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "903898f2c4c2bd213031a25405b528451e328f2929926e7b14cdc655bc7df52f" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 5003 "dfe46201a7f1245eb2dbcede5f4af89ec0f534ea676fef15ec50879db42c1ede" "600"
check_and_install_artifact "sp8c7a_manifest.json" 2742 "9527d35c6ed33149c284d317bcd2ed6884a89127f974b19ce6ea2d9a969cf282" "600"
...
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 50211 "9ee0a6014aff9c2eb70c1e899cbc9c9e33bc98919a57add1b193a9871a769138" "1001:1001" "700"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "903898f2c4c2bd213031a25405b528451e328f2929926e7b14cdc655bc7df52f" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 5003 "dfe46201a7f1245eb2dbcede5f4af89ec0f534ea676fef15ec50879db42c1ede" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 2742 "9527d35c6ed33149c284d317bcd2ed6884a89127f974b19ce6ea2d9a969cf282" "1001:1001" "600"
...
set +e
EXPECTED_SP8C7A_SHA256="9ee0a6014aff9c2eb70c1e899cbc9c9e33bc98919a57add1b193a9871a769138" \
EXPECTED_SP8C7A_BYTES="50211" \
...
# Require both PREFLIGHT_STATUS == 0 and outer TEE_STATUS == 0
if [ "${PREFLIGHT_STATUS}" -ne 0 ] || [ "${TEE_STATUS}" -ne 0 ]; then
    echo "CRITICAL ERROR: Preflight script (status: ${PREFLIGHT_STATUS}) or Tee (status: ${TEE_STATUS}) returned non-zero exit code!" >&2
    exit 1
fi

# Extract exactly one canonical-log declaration from the current execution output
LOG_DECL_LINES="$(grep "^>>> Canonical Log" "${EXEC_OUT_FILE}" || true)"
if [ -z "${LOG_DECL_LINES}" ]; then
    LOG_DECL_COUNT=0
else
...
# Treat missing, symlinked, incorrectly owned, incorrectly permissioned, or unverifiable canonical log as fatal error
if [ -z "${CANON_LOG}" ]; then
    echo "CRITICAL ERROR: Failed to parse canonical log path from execution output!" >&2
    exit 1
fi
...
case "${CANON_LOG}" in
    ...
    *)
        echo "CRITICAL ERROR: Canonical log path has invalid format/prefix: ${CANON_LOG}" >&2
        exit 1
        ;;
esac

canon_resolved_log="$(readlink -f "${CANON_LOG}" 2>/dev/null || true)"
if [ "${canon_resolved_log}" != "${CANON_LOG}" ]; then
    echo "CRITICAL ERROR: Canonical log path does not match resolved canonical path: ${canon_resolved_log}" >&2
    exit 1
fi
...
echo "========================================================================"
echo "Gate SP-8C-7A Revision R11 host synchronization and preflight execution PASSED with exit code 0."
exit 0
