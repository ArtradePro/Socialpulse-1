...
# GATE SP-8C-7A: SYNCHRONIZATION AND EXECUTION WRAPPER (REVISION R21)
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A WRAPPER (REVISION R21)"
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R21)
EXPECTED_SCRIPT_SHA256="d888624b5e1f41135942df99471823ca1b82afa98b9aeea8e7c62ba8b982f778"
EXPECTED_SCRIPT_BYTES=61362
EXPECTED_SCRIPT_LINES=1460

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R21)
EXPECTED_SIDECAR_SHA256="39af962da0624ae808087fbb60b3b360c5a82b1e94ff87f76563e4cabdca44f2"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R21)
EXPECTED_RUNBOOK_SHA256="a5c149928eb6d6934024752027b212a7e05386f8088ad807bdc847173381211c"
EXPECTED_RUNBOOK_BYTES=16791
EXPECTED_RUNBOOK_LINES=166

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R21)
EXPECTED_MANIFEST_SHA256="8cee8dcd79f5675209cf5123c0a61ae7982bd5bdd708ed513cffce018d3961d3"
EXPECTED_MANIFEST_BYTES=5758
EXPECTED_MANIFEST_LINES=119
    ...
    if [ "${PUBLICATION_STATE}" = "UNCOMMITTED" ]; then
        ...
        for (( idx=num_replaced-1; idx>=0; idx-- )); do
            ...
            if [ "${published}" -eq 1 ]; then
                ...
                # Destination was never replaced; remove unused backup if present
...
echo ""
echo "========================================================================"
echo ">>> [2/6] Staging Gate SP-8C-7A Revision R21 Approved Artifacts"
echo "========================================================================"
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 61362 "d888624b5e1f41135942df99471823ca1b82afa98b9aeea8e7c62ba8b982f778"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "39af962da0624ae808087fbb60b3b360c5a82b1e94ff87f76563e4cabdca44f2"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 16791 "a5c149928eb6d6934024752027b212a7e05386f8088ad807bdc847173381211c"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 5758 "8cee8dcd79f5675209cf5123c0a61ae7982bd5bdd708ed513cffce018d3961d3"
...
assert rev == "R21", f"revision expected R21, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 61362,
        "lines": 1460,
        "sha256": "d888624b5e1f41135942df99471823ca1b82afa98b9aeea8e7c62ba8b982f778",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "39af962da0624ae808087fbb60b3b360c5a82b1e94ff87f76563e4cabdca44f2",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 16791,
        "lines": 166,
        "sha256": "a5c149928eb6d6934024752027b212a7e05386f8088ad807bdc847173381211c",
        "role": "operational_preflight_runbook_and_invariants_revision_r21"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R21, exactly 3 payload members with exact field values."
...
check_and_install_artifact "sp8c7a_preflight.sh" 61362 "d888624b5e1f41135942df99471823ca1b82afa98b9aeea8e7c62ba8b982f778" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "39af962da0624ae808087fbb60b3b360c5a82b1e94ff87f76563e4cabdca44f2" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 16791 "a5c149928eb6d6934024752027b212a7e05386f8088ad807bdc847173381211c" "600"
check_and_install_artifact "sp8c7a_manifest.json" 5758 "8cee8dcd79f5675209cf5123c0a61ae7982bd5bdd708ed513cffce018d3961d3" "600"
...
commit_and_cleanup_publication() {
    ...
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 61362 "d888624b5e1f41135942df99471823ca1b82afa98b9aeea8e7c62ba8b982f778" "1001:1001" "700"
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "39af962da0624ae808087fbb60b3b360c5a82b1e94ff87f76563e4cabdca44f2" "1001:1001" "600"
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 16791 "a5c149928eb6d6934024752027b212a7e05386f8088ad807bdc847173381211c" "1001:1001" "600"
    reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 5758 "8cee8dcd79f5675209cf5123c0a61ae7982bd5bdd708ed513cffce018d3961d3" "1001:1001" "600"
...
set +e
EXPECTED_SP8C7A_SHA256="d888624b5e1f41135942df99471823ca1b82afa98b9aeea8e7c62ba8b982f778" \
