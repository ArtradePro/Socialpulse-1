...
# GATE SP-8C-7A: SYNCHRONIZATION AND EXECUTION WRAPPER (REVISION R20)
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A WRAPPER (REVISION R20)"
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R20)
EXPECTED_SCRIPT_SHA256="ca7ed5f03073970d5dc025019090182b1b4c83d7b1c0078d21eb5bdc2a024a98"
EXPECTED_SCRIPT_BYTES=61362
EXPECTED_SCRIPT_LINES=1460

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R20)
EXPECTED_SIDECAR_SHA256="3d694234da29cfccc6f4b8cedc1a7156feac59cc7458c466e5343a0f79be03fe"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R20)
EXPECTED_RUNBOOK_SHA256="c5e1cb2a7bfffbc14f8c130e6920da3cd67c1265d2803b63b1b336a9d48b4020"
EXPECTED_RUNBOOK_BYTES=15240
EXPECTED_RUNBOOK_LINES=153

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R20)
EXPECTED_MANIFEST_SHA256="e825aa5c688b014a8ff2af4f43dba5a2dce94a09e6b165b9576ac22007f3052a"
EXPECTED_MANIFEST_BYTES=5562
EXPECTED_MANIFEST_LINES=116
    ...
    if [ "${PUBLICATION_COMMITTED}" -eq 0 ]; then
        ...
        for (( idx=num_replaced-1; idx>=0; idx-- )); do
            ...
            if [ "${published}" -eq 1 ]; then
                # Destination was replaced; must restore from verified backup
                if [ -z "${bak}" ] || [ ! -f "${bak}" ]; then
                ...
                # Destination was never replaced; if backup exists, remove it and verify absence
...
echo ""
echo "========================================================================"
echo ">>> [2/6] Staging Gate SP-8C-7A Revision R20 Approved Artifacts"
echo "========================================================================"
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 61362 "ca7ed5f03073970d5dc025019090182b1b4c83d7b1c0078d21eb5bdc2a024a98"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "3d694234da29cfccc6f4b8cedc1a7156feac59cc7458c466e5343a0f79be03fe"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 15240 "c5e1cb2a7bfffbc14f8c130e6920da3cd67c1265d2803b63b1b336a9d48b4020"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 5562 "e825aa5c688b014a8ff2af4f43dba5a2dce94a09e6b165b9576ac22007f3052a"
...
assert rev == "R20", f"revision expected R20, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 61362,
        "lines": 1460,
        "sha256": "ca7ed5f03073970d5dc025019090182b1b4c83d7b1c0078d21eb5bdc2a024a98",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "3d694234da29cfccc6f4b8cedc1a7156feac59cc7458c466e5343a0f79be03fe",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 15240,
        "lines": 153,
        "sha256": "c5e1cb2a7bfffbc14f8c130e6920da3cd67c1265d2803b63b1b336a9d48b4020",
        "role": "operational_preflight_runbook_and_invariants_revision_r20"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R20, exactly 3 payload members with exact field values."
...
check_and_install_artifact "sp8c7a_preflight.sh" 61362 "ca7ed5f03073970d5dc025019090182b1b4c83d7b1c0078d21eb5bdc2a024a98" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "3d694234da29cfccc6f4b8cedc1a7156feac59cc7458c466e5343a0f79be03fe" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 15240 "c5e1cb2a7bfffbc14f8c130e6920da3cd67c1265d2803b63b1b336a9d48b4020" "600"
check_and_install_artifact "sp8c7a_manifest.json" 5562 "e825aa5c688b014a8ff2af4f43dba5a2dce94a09e6b165b9576ac22007f3052a" "600"

echo ""
echo "========================================================================"
echo ">>> [5/6] Post-Publication Reverification, Cleanup & Commit Boundary"
echo "========================================================================"
...
commit_publication_transaction() {
    echo ">>> Reverifying all 4 published destinations before commit..."

    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 61362 "ca7ed5f03073970d5dc025019090182b1b4c83d7b1c0078d21eb5bdc2a024a98" "1001:1001" "700"
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "3d694234da29cfccc6f4b8cedc1a7156feac59cc7458c466e5343a0f79be03fe" "1001:1001" "600"
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 15240 "c5e1cb2a7bfffbc14f8c130e6920da3cd67c1265d2803b63b1b336a9d48b4020" "1001:1001" "600"
    reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 5562 "e825aa5c688b014a8ff2af4f43dba5a2dce94a09e6b165b9576ac22007f3052a" "1001:1001" "600"

    echo ">>> All destination files reverified. Removing replacement backups & verifying absence..."

    local cleanup_error=0
    # Clean every exact tracked backup path and verify absence
    ...
    # Clean every tracked temporary staging file and verify absence
    for tmp in "${TRACKED_TEMP_FILES[@]}"; do
        if [ -e "${tmp}" ] || [ -L "${tmp}" ]; then
            rm -f "${tmp}"
            if [ -e "${tmp}" ] || [ -L "${tmp}" ]; then
                echo "CRITICAL ERROR: Temporary staging file remained: ${tmp}!" >&2
                cleanup_error=1
            else
    ...
    if [ "${cleanup_error}" -ne 0 ]; then
        echo "CRITICAL ERROR: Post-publication cleanup failed! Refusing to commit transaction." >&2
        exit 1
    fi

    # All destinations verified, all backups removed and verified absent, all temp files verified absent, zero containment errors
    PUBLICATION_COMMITTED=1
    echo "Publication transaction committed successfully."
}

# Execute the publication commit function
commit_publication_transaction
...
set +e
EXPECTED_SP8C7A_SHA256="ca7ed5f03073970d5dc025019090182b1b4c83d7b1c0078d21eb5bdc2a024a98" \
...
echo "Publication Transaction      : COMMITTED (All backups removed & verified absent)"
