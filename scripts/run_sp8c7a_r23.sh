...
# GATE SP-8C-7A: SYNCHRONIZATION AND EXECUTION WRAPPER (REVISION R23)
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A WRAPPER (REVISION R23)"
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R23)
EXPECTED_SCRIPT_SHA256="76ad215b03ec619796d08ffa01b368f0943c4e0b646c4ba3f40bbbd9cbecef1e"
EXPECTED_SCRIPT_BYTES=63752
EXPECTED_SCRIPT_LINES=1515

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R23)
EXPECTED_SIDECAR_SHA256="9ff66433cc4a68d10511ca81b2bf27951f4e5f3e9862465c402224db225e51a3"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R23)
EXPECTED_RUNBOOK_SHA256="7f6434508bb38a67759f49e5636e7deeb862919547ad76d15e8b22f3209fe9f8"
EXPECTED_RUNBOOK_BYTES=15889
EXPECTED_RUNBOOK_LINES=147

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R23)
EXPECTED_MANIFEST_SHA256="d29165297928fe19148fd1d341266a33585cfeec5dbbfdcee5042a0fcde38ad4"
EXPECTED_MANIFEST_BYTES=6164
EXPECTED_MANIFEST_LINES=127
    ...
    if [ "${PUBLICATION_STATE}" = "UNCOMMITTED" ]; then
        ...
        for (( idx=num_replaced-1; idx>=0; idx-- )); do
            ...
            if [ "${published}" -eq 1 ]; then
                ...
                r_sha="$(sha256sum -- "${dest}" | cut -d' ' -f1)"
                if ! [[ "${r_sha}" =~ ^[0-9a-f]{64}$ ]]; then r_clean_failed=1; fi
...
# Create execution output file with strict root permissions (0:0, 0600) for fs.protected_regular compliance
EXEC_OUT_FILE="$(mktemp /tmp/sp8c7a_exec_out.XXXXXX)"
chown 0:0 "${EXEC_OUT_FILE}"
chmod 0600 "${EXEC_OUT_FILE}"

if [ ! -f "${EXEC_OUT_FILE}" ] || [ -L "${EXEC_OUT_FILE}" ]; then
    echo "CRITICAL ERROR: Execution output file is missing or a symlink: ${EXEC_OUT_FILE}" >&2
    exit 1
fi
if [ "$(stat -c '%u:%g' "${EXEC_OUT_FILE}")" != "0:0" ]; then
    echo "CRITICAL ERROR: Execution output file ownership mismatch (expected 0:0): ${EXEC_OUT_FILE}" >&2
    exit 1
fi
if [ "$(stat -c '%a' "${EXEC_OUT_FILE}")" != "600" ]; then
    echo "CRITICAL ERROR: Execution output file permissions mismatch (expected 600): ${EXEC_OUT_FILE}" >&2
    exit 1
fi
case "${EXEC_OUT_FILE}" in
    /tmp/sp8c7a_exec_out.*) ;;
    *)
        echo "CRITICAL ERROR: Execution output file violates prefix /tmp/sp8c7a_exec_out.*: ${EXEC_OUT_FILE}" >&2
        exit 1
        ;;
esac
echo "Execution output file created: ${EXEC_OUT_FILE} (owner 0:0, mode 0600)"

echo ""
echo "========================================================================"
echo ">>> [2/6] Staging Gate SP-8C-7A Revision R23 Approved Artifacts"
echo "========================================================================"
...
verify_staged_artifact() {
    ...
    local act_sha256
    act_sha256="$(sha256sum -- "${target}" | cut -d' ' -f1)"
    if ! [[ "${act_sha256}" =~ ^[0-9a-f]{64}$ ]]; then echo "CRITICAL ERROR: Invalid SHA-256 for ${target}!" >&2; exit 1; fi
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 63752 "76ad215b03ec619796d08ffa01b368f0943c4e0b646c4ba3f40bbbd9cbecef1e"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "9ff66433cc4a68d10511ca81b2bf27951f4e5f3e9862465c402224db225e51a3"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 15889 "7f6434508bb38a67759f49e5636e7deeb862919547ad76d15e8b22f3209fe9f8"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 6164 "d29165297928fe19148fd1d341266a33585cfeec5dbbfdcee5042a0fcde38ad4"
...
assert rev == "R23", f"revision expected R22, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 63752,
        "lines": 1515,
        "sha256": "76ad215b03ec619796d08ffa01b368f0943c4e0b646c4ba3f40bbbd9cbecef1e",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "9ff66433cc4a68d10511ca81b2bf27951f4e5f3e9862465c402224db225e51a3",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 15889,
        "lines": 147,
        "sha256": "7f6434508bb38a67759f49e5636e7deeb862919547ad76d15e8b22f3209fe9f8",
        "role": "operational_preflight_runbook_and_invariants_revision_r23"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R23, exactly 3 payload members with exact field values."
...
check_and_install_artifact() {
    ...
    if [ -e "${dest_file}" ]; then
        ...
        act_sha256="$(sha256sum -- "${dest_file}" | cut -d' ' -f1)"
        if ! [[ "${act_sha256}" =~ ^[0-9a-f]{64}$ ]]; then echo "CRITICAL ERROR: Invalid SHA-256 for ${dest_file}!" >&2; exit 1; fi
        ...
        bak_sha256="$(sha256sum -- "${backup_file}" | cut -d' ' -f1)"
        if ! [[ "${bak_sha256}" =~ ^[0-9a-f]{64}$ ]]; then echo "CRITICAL ERROR: Invalid SHA-256 for ${backup_file}!" >&2; exit 1; fi
    ...
    tmp_sha256="$(sha256sum -- "${tmp_dest}" | cut -d' ' -f1)"
    if ! [[ "${tmp_sha256}" =~ ^[0-9a-f]{64}$ ]]; then echo "CRITICAL ERROR: Invalid SHA-256 for ${tmp_dest}!" >&2; exit 1; fi
...
check_and_install_artifact "sp8c7a_preflight.sh" 63752 "76ad215b03ec619796d08ffa01b368f0943c4e0b646c4ba3f40bbbd9cbecef1e" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "9ff66433cc4a68d10511ca81b2bf27951f4e5f3e9862465c402224db225e51a3" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 15889 "7f6434508bb38a67759f49e5636e7deeb862919547ad76d15e8b22f3209fe9f8" "600"
check_and_install_artifact "sp8c7a_manifest.json" 6164 "d29165297928fe19148fd1d341266a33585cfeec5dbbfdcee5042a0fcde38ad4" "600"
...
commit_and_cleanup_publication() {
    ...
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 63752 "76ad215b03ec619796d08ffa01b368f0943c4e0b646c4ba3f40bbbd9cbecef1e" "1001:1001" "700"
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "9ff66433cc4a68d10511ca81b2bf27951f4e5f3e9862465c402224db225e51a3" "1001:1001" "600"
    reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 15889 "7f6434508bb38a67759f49e5636e7deeb862919547ad76d15e8b22f3209fe9f8" "1001:1001" "600"
    reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 6164 "d29165297928fe19148fd1d341266a33585cfeec5dbbfdcee5042a0fcde38ad4" "1001:1001" "600"
...
set +e
EXPECTED_SP8C7A_SHA256="76ad215b03ec619796d08ffa01b368f0943c4e0b646c4ba3f40bbbd9cbecef1e" \
EXPECTED_SP8C7A_BYTES="63752" \
...
if ! [[ "${LOG_SHA256}" =~ ^[0-9a-f]{64}$ ]]; then echo "CRITICAL ERROR: Invalid SHA-256 for ${LOG_PATH}!" >&2; exit 1; fi
