...
# GATE SP-8C-7A REVISION R7: HOST STAGING, VERIFICATION & PREFLIGHT WRAPPER
# Target Host: srv1935605 (2.24.130.251)
# Script Name: run_sp8c7a_r7.sh
...
echo ""
echo "========================================================================"
echo ">>> [2/6] Downloading Approved Gate SP-8C-7A R7 Artifacts into Staging"
echo "========================================================================"
...
if [ ! -f "${STAGE_DIR}/sp8c7a_manifest.json" ] || [ -L "${STAGE_DIR}/sp8c7a_manifest.json" ]; then
    echo "CRITICAL ERROR: Staged sp8c7a_manifest.json is missing, non-regular, or a symlink." >&2
    exit 1
fi
echo "Staged artifact verified: sp8c7a_manifest.json (regular non-symlink file)"
...
wrapper_script_path = sys.argv[2]
...
assert rev == "R7", f"revision expected R7, got {rev}"
...
for m in members:
    ...
    elif fn == "run_sp8c7a_r7.sh":
        assert m["role"] == "governed_host_synchronization_and_preflight_execution_wrapper", "Wrapper role mismatch"
        with open(wrapper_script_path, "rb") as wf:
            wf_bytes = wf.read()
        act_w_sha = hashlib.sha256(wf_bytes).hexdigest()
        act_w_len = len(wf_bytes)
        act_w_lines = wf_bytes.count(b"\n")
        m_sha = m["sha256"]
        m_b = m["bytes"]
        m_l = m["lines"]
        assert m_sha == act_w_sha, f"run_sp8c7a_r7.sh sha256 mismatch: manifest {m_sha} vs runtime {act_w_sha}"
        assert m_b == act_w_len, f"run_sp8c7a_r7.sh bytes mismatch: manifest {m_b} vs runtime {act_w_len}"
        assert m_l == act_w_lines, f"run_sp8c7a_r7.sh lines mismatch: manifest {m_l} vs runtime {act_w_lines}"
    else:
        raise AssertionError(f"Unexpected member filename: {fn}")
' "${STAGE_DIR}/sp8c7a_manifest.json" "$0" || PYTHON_MANIFEST_STATUS=$?

if [ "${PYTHON_MANIFEST_STATUS}" -ne 0 ]; then
    echo "CRITICAL ERROR: Python manifest validation failed (status ${PYTHON_MANIFEST_STATUS})" >&2
    exit 1
fi
echo "Staged manifest verified: schema 1.0, SP-8C-7A, R7, exactly 4 members with exact field values."
    ...
    if [ -e "${dest_file}" ]; then
        ...
        if [ "${act_bytes}" -eq "${exp_bytes}" ] && \
           ...
           [ "${act_mode}" = "${exp_mode}" ]; then
            echo "Destination ${fname} already exists and matches the approved R7 artifact exactly (preserving without re-installation)."
            return 0
        else
            echo "CRITICAL ERROR: Destination collision at ${dest_file}! Existing file does not match approved R7 artifact." >&2
...
# Install manifest directly with strict permissions (verified during Python validation)
if [ -L "${DEST_DIR}/sp8c7a_manifest.json" ]; then
    echo "CRITICAL ERROR: Destination ${DEST_DIR}/sp8c7a_manifest.json is a symlink. Symlinks rejected." >&2
    exit 1
fi
install -m 0600 -o 1001 -g 1001 "${STAGE_DIR}/sp8c7a_manifest.json" "${DEST_DIR}/sp8c7a_manifest.json"
CREATED_BY_WRAPPER+=("${DEST_DIR}/sp8c7a_manifest.json")
echo "Installed sp8c7a_manifest.json (owner 1001:1001, mode 0600)"
