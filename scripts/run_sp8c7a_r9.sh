...
# GATE SP-8C-7A: REVISION R9 HOST SYNCHRONIZATION & PREFLIGHT EXECUTION WRAPPER
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A REVISION R9 HOST WRAPPER"
...
if [ "$(id -u)" -ne 0 ]; then
    echo "CRITICAL ERROR: run_sp8c7a_r9.sh must be executed as root (EUID 0)!" >&2
    exit 1
fi
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R9)
EXPECTED_SCRIPT_SHA256="b7a94e40b64bfea596d11460ee2a8c3f074cb09b12c46c3866ecab3c125f6db5"
EXPECTED_SCRIPT_BYTES=48336
EXPECTED_SCRIPT_LINES=1166

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R9)
EXPECTED_SIDECAR_SHA256="0730aeeaaeee7e0b5ca61118e37d59ec0ac770c9ea4f56257e27285284fc88b0"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R9)
EXPECTED_RUNBOOK_SHA256="edc152b94e73069513d533522fe74401e91f611c72628fe2d651a9b0a1975bd5"
EXPECTED_RUNBOOK_BYTES=5000
EXPECTED_RUNBOOK_LINES=43

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R9)
EXPECTED_MANIFEST_SHA256="ee5f5bdd8760ca4ae7ca0a5b6c87dcbccde3e78f700a29f746a477faf8a0756e"
EXPECTED_MANIFEST_BYTES=2740
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 48336 "b7a94e40b64bfea596d11460ee2a8c3f074cb09b12c46c3866ecab3c125f6db5"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "0730aeeaaeee7e0b5ca61118e37d59ec0ac770c9ea4f56257e27285284fc88b0"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 5000 "edc152b94e73069513d533522fe74401e91f611c72628fe2d651a9b0a1975bd5"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 2740 "ee5f5bdd8760ca4ae7ca0a5b6c87dcbccde3e78f700a29f746a477faf8a0756e"
...
assert rev == "R9", f"revision expected R9, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 48336,
        "lines": 1166,
        "sha256": "b7a94e40b64bfea596d11460ee2a8c3f074cb09b12c46c3866ecab3c125f6db5",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "0730aeeaaeee7e0b5ca61118e37d59ec0ac770c9ea4f56257e27285284fc88b0",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 5000,
        "lines": 43,
        "sha256": "edc152b94e73069513d533522fe74401e91f611c72628fe2d651a9b0a1975bd5",
        "role": "operational_preflight_runbook_and_invariants_revision_r9"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R9, exactly 3 payload members with exact field values."
...
check_and_install_artifact "sp8c7a_preflight.sh" 48336 "b7a94e40b64bfea596d11460ee2a8c3f074cb09b12c46c3866ecab3c125f6db5" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "0730aeeaaeee7e0b5ca61118e37d59ec0ac770c9ea4f56257e27285284fc88b0" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 5000 "edc152b94e73069513d533522fe74401e91f611c72628fe2d651a9b0a1975bd5" "600"
check_and_install_artifact "sp8c7a_manifest.json" 2740 "ee5f5bdd8760ca4ae7ca0a5b6c87dcbccde3e78f700a29f746a477faf8a0756e" "600"
...
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 48336 "b7a94e40b64bfea596d11460ee2a8c3f074cb09b12c46c3866ecab3c125f6db5" "1001:1001" "700"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "0730aeeaaeee7e0b5ca61118e37d59ec0ac770c9ea4f56257e27285284fc88b0" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 5000 "edc152b94e73069513d533522fe74401e91f611c72628fe2d651a9b0a1975bd5" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 2740 "ee5f5bdd8760ca4ae7ca0a5b6c87dcbccde3e78f700a29f746a477faf8a0756e" "1001:1001" "600"
...
set +e
EXPECTED_SP8C7A_SHA256="b7a94e40b64bfea596d11460ee2a8c3f074cb09b12c46c3866ecab3c125f6db5" \
...
# Extract canonical log path
LOG_LINE="$(grep "Canonical Log" "${EXEC_OUT_FILE}" | head -n 1 || true)"
CANON_LOG=""
if [ -n "${LOG_LINE}" ]; then
    CANON_LOG="$(echo "${LOG_LINE}" | awk '{print $4}')"
fi

if [ -z "${CANON_LOG}" ]; then
    # Fallback to discover timestamped log file strictly with exact regex and prefix
    echo "Warning: Unable to parse canonical log line from stdout; searching /root/ for exact sp8c7a_preflight log..."
    CANON_LOG="$(find /root -maxdepth 1 -name "sp8c7a_preflight_*.log" -type f ! -type l -printf '%T@ %p\n' 2>/dev/null | sort -nr | head -n 1 | awk '{print $2}')"
fi

if [ -n "${CANON_LOG}" ] && [ -f "${CANON_LOG}" ] && [ ! -L "${CANON_LOG}" ]; then
    ...
    echo "CRITICAL WARNING: Canonical root preflight log could not be verified!" >&2
fi

if [ "${PREFLIGHT_STATUS}" -ne 0 ]; then
    echo "CRITICAL: SP-8C-7A Preflight failed with exit status ${PREFLIGHT_STATUS}!" >&2
    exit "${PREFLIGHT_STATUS}"
fi

echo "Gate SP-8C-7A Revision R9 host synchronization and preflight execution PASSED with exit code 0."
exit 0
