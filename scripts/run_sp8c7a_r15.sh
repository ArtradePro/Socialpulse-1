...
# GATE SP-8C-7A: REVISION R15 HOST SYNCHRONIZATION & PREFLIGHT EXECUTION WRAPPER
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A REVISION R15 HOST WRAPPER"
...
if [ "$(id -u)" -ne 0 ]; then
    echo "CRITICAL ERROR: run_sp8c7a_r15.sh must be executed as root (EUID 0)!" >&2
    exit 1
fi
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R15)
EXPECTED_SCRIPT_SHA256="cb073aa909c35d0dfee20e759447f0e94b1fff1e6591c14439423814b10987f0"
EXPECTED_SCRIPT_BYTES=53181
EXPECTED_SCRIPT_LINES=1290

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R15)
EXPECTED_SIDECAR_SHA256="18f85a3ac74e4a5ec88733965987a46b79ba0a739103c31ef69a0462c5e09733"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R15)
EXPECTED_RUNBOOK_SHA256="569fcc6b74f84ad35a74c73a53269fdce8c661a6293eaf513bd1b4488ef9ab8a"
EXPECTED_RUNBOOK_BYTES=8995
EXPECTED_RUNBOOK_LINES=90

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R15)
EXPECTED_MANIFEST_SHA256="2141cc181e5f9f946513bf6a9239924440412ab63fd97c2710acc9263e0eccc9"
EXPECTED_MANIFEST_BYTES=4225
EXPECTED_MANIFEST_LINES=91
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 53181 "cb073aa909c35d0dfee20e759447f0e94b1fff1e6591c14439423814b10987f0"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "18f85a3ac74e4a5ec88733965987a46b79ba0a739103c31ef69a0462c5e09733"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 8995 "569fcc6b74f84ad35a74c73a53269fdce8c661a6293eaf513bd1b4488ef9ab8a"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 4225 "2141cc181e5f9f946513bf6a9239924440412ab63fd97c2710acc9263e0eccc9"
...
assert rev == "R15", f"revision expected R15, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 53181,
        "lines": 1290,
        "sha256": "cb073aa909c35d0dfee20e759447f0e94b1fff1e6591c14439423814b10987f0",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "18f85a3ac74e4a5ec88733965987a46b79ba0a739103c31ef69a0462c5e09733",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 8995,
        "lines": 90,
        "sha256": "569fcc6b74f84ad35a74c73a53269fdce8c661a6293eaf513bd1b4488ef9ab8a",
        "role": "operational_preflight_runbook_and_invariants_revision_r15"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R15, exactly 3 payload members with exact field values."
...
check_and_install_artifact "sp8c7a_preflight.sh" 53181 "cb073aa909c35d0dfee20e759447f0e94b1fff1e6591c14439423814b10987f0" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "18f85a3ac74e4a5ec88733965987a46b79ba0a739103c31ef69a0462c5e09733" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 8995 "569fcc6b74f84ad35a74c73a53269fdce8c661a6293eaf513bd1b4488ef9ab8a" "600"
check_and_install_artifact "sp8c7a_manifest.json" 4225 "2141cc181e5f9f946513bf6a9239924440412ab63fd97c2710acc9263e0eccc9" "600"
...
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 53181 "cb073aa909c35d0dfee20e759447f0e94b1fff1e6591c14439423814b10987f0" "1001:1001" "700"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "18f85a3ac74e4a5ec88733965987a46b79ba0a739103c31ef69a0462c5e09733" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 8995 "569fcc6b74f84ad35a74c73a53269fdce8c661a6293eaf513bd1b4488ef9ab8a" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 4225 "2141cc181e5f9f946513bf6a9239924440412ab63fd97c2710acc9263e0eccc9" "1001:1001" "600"
...
set +e
EXPECTED_SP8C7A_SHA256="cb073aa909c35d0dfee20e759447f0e94b1fff1e6591c14439423814b10987f0" \
EXPECTED_SP8C7A_BYTES="53181" \
...
if [ "${PREFLIGHT_STATUS}" -eq 0 ]; then
    echo "Gate SP-8C-7A Revision R15 host synchronization and preflight execution PASSED with PREFLIGHT_COMPLETE_NO_FINDINGS (exit code 0)."
    exit 0
elif [ "${PREFLIGHT_STATUS}" -eq 2 ]; then
    echo "Gate SP-8C-7A Revision R15 host synchronization and preflight execution CONCLUDED with PREFLIGHT_COMPLETE_WITH_FINDINGS (exit code 2)."
    echo "Findings documented. Migration execution remains strictly prohibited pending resolution."
    exit 2
fi
