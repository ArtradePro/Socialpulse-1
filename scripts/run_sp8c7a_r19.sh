...
# GATE SP-8C-7A: REVISION R19 HOST SYNCHRONIZATION & PREFLIGHT EXECUTION WRAPPER
# Identity: root (EUID 0) on srv1935605 (2.24.130.251)
# Governed Path: /opt/socialpulse/scripts/ (owner 1001:1001, mode 0755)
# Scope: Controlled download, integrity validation, transactional publication with
#        verified pre-replacement backup, atomic same-filesystem rename,
#        deterministic rollback, and read-only preflight execution.
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A REVISION R19 HOST WRAPPER"
...
if [ "$(id -u)" -ne 0 ]; then
    echo "CRITICAL ERROR: run_sp8c7a_r19.sh must be executed as root (EUID 0)!" >&2
    exit 1
fi
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R19)
EXPECTED_SCRIPT_SHA256="81ab86857bb50dfcbfb41833ae77e7bc96db32730741033ea5b0e2e870ef143f"
EXPECTED_SCRIPT_BYTES=61362
EXPECTED_SCRIPT_LINES=1460

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R19)
EXPECTED_SIDECAR_SHA256="95638fbe712b44cc45119cb8f83b7072b6d853d7934de5475bc0784c39072123"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R19)
EXPECTED_RUNBOOK_SHA256="de9893ef63c1aad7a0461b18969c494e424c443d754d2d8e38a372ae0a774352"
EXPECTED_RUNBOOK_BYTES=13787
EXPECTED_RUNBOOK_LINES=141

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R19)
EXPECTED_MANIFEST_SHA256="6e39e06130ce9a8011f77cac2cbffd918835c013beb1ad9a93d6e3911679cc88"
EXPECTED_MANIFEST_BYTES=5387
EXPECTED_MANIFEST_LINES=112
...
rollback_publication() {
    ...
    if [ "${PUBLICATION_COMMITTED}" -eq 0 ]; then
        echo ">>> Initiating transactional rollback of uncommitted destination modifications..." >&2

        # 1. Delete only newly created destinations
        ...
        # 2. Atomically restore every replaced destination from verified backup
        for dest in "${REPLACED_DESTINATIONS[@]}"; do
            local bak="${DEST_BACKUP_MAP[${dest}]:-}"
            if [ -z "${bak}" ] || [ ! -f "${bak}" ]; then
                echo "CRITICAL ERROR: Backup file missing for replaced destination ${dest}!" >&2
                rollback_failed=1
                continue
            fi
            ...
            if [ "${r_bytes}" -ne "${exp_b}" ] || \
               ...
               [ -L "${dest}" ]; then
                ...
                echo "Successfully restored original destination: ${dest##*/} (bytes: ${r_bytes}, sha: ${r_sha}, owner: ${r_owner}, mode: 0${r_mode})" >&2
            fi
        done
    fi

    # Clean any remaining temp files in DEST_DIR
    rm -f "${DEST_DIR}/.tmp_"* 2>/dev/null || true

    if [ "${rollback_failed}" -ne 0 ]; then
        echo "CRITICAL ERROR: Containment failure during publication rollback!" >&2
        return 1
    fi
    return 0
}
...
on_exit() {
    ...
    rollback_publication || exit_code=1
...
on_signal() {
    ...
    rollback_publication || clean_failed=1
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 61362 "81ab86857bb50dfcbfb41833ae77e7bc96db32730741033ea5b0e2e870ef143f"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "95638fbe712b44cc45119cb8f83b7072b6d853d7934de5475bc0784c39072123"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 13787 "de9893ef63c1aad7a0461b18969c494e424c443d754d2d8e38a372ae0a774352"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 5387 "6e39e06130ce9a8011f77cac2cbffd918835c013beb1ad9a93d6e3911679cc88"
...
assert rev == "R19", f"revision expected R19, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 61362,
        "lines": 1460,
        "sha256": "81ab86857bb50dfcbfb41833ae77e7bc96db32730741033ea5b0e2e870ef143f",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "95638fbe712b44cc45119cb8f83b7072b6d853d7934de5475bc0784c39072123",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 13787,
        "lines": 141,
        "sha256": "de9893ef63c1aad7a0461b18969c494e424c443d754d2d8e38a372ae0a774352",
        "role": "operational_preflight_runbook_and_invariants_revision_r19"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R19, exactly 3 payload members with exact field values."
...
check_and_install_artifact() {
    ...
    if [ -e "${dest_file}" ]; then
        ...
        echo "Destination ${fname} exists with differing contents; creating verified backup before replacement..."
    ...
    # Publish with atomic same-filesystem rename
    mv -f "${tmp_dest}" "${dest_file}"
    echo "Atomically published ${fname} (owner 1001:1001, mode 0${exp_mode}, bytes ${exp_bytes})"
}

check_and_install_artifact "sp8c7a_preflight.sh" 61362 "81ab86857bb50dfcbfb41833ae77e7bc96db32730741033ea5b0e2e870ef143f" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "95638fbe712b44cc45119cb8f83b7072b6d853d7934de5475bc0784c39072123" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 13787 "de9893ef63c1aad7a0461b18969c494e424c443d754d2d8e38a372ae0a774352" "600"
check_and_install_artifact "sp8c7a_manifest.json" 5387 "6e39e06130ce9a8011f77cac2cbffd918835c013beb1ad9a93d6e3911679cc88" "600"
...
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 61362 "81ab86857bb50dfcbfb41833ae77e7bc96db32730741033ea5b0e2e870ef143f" "1001:1001" "700"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "95638fbe712b44cc45119cb8f83b7072b6d853d7934de5475bc0784c39072123" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 13787 "de9893ef63c1aad7a0461b18969c494e424c443d754d2d8e38a372ae0a774352" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 5387 "6e39e06130ce9a8011f77cac2cbffd918835c013beb1ad9a93d6e3911679cc88" "1001:1001" "600"

PUBLICATION_COMMITTED=1
echo "Publication transaction committed successfully."

# Remove replacement backups only after all four destination files pass post-publication verification and publication is committed
for dest in "${REPLACED_DESTINATIONS[@]}"; do
    local bak="${DEST_BACKUP_MAP[${dest}]:-}"
    if [ -n "${bak}" ] && [ -e "${bak}" ]; then
        rm -f "${bak}"
        if [ -e "${bak}" ] || [ -L "${bak}" ]; then
            echo "CRITICAL ERROR: Failed to remove backup file: ${bak}!" >&2
            exit 1
        fi
        echo "Cleaned up replacement backup: ${bak##*/}"
    fi
done
...
set +e
EXPECTED_SP8C7A_SHA256="81ab86857bb50dfcbfb41833ae77e7bc96db32730741033ea5b0e2e870ef143f" \
...
if [ "${PREFLIGHT_STATUS}" -eq 0 ]; then
    ...
    echo "Gate SP-8C-7A Revision R19 host synchronization and preflight execution PASSED with PREFLIGHT_COMPLETE_NO_FINDINGS (exit code 0)."
    exit 0
    ...
    echo "Gate SP-8C-7A Revision R19 host synchronization and preflight execution CONCLUDED with PREFLIGHT_COMPLETE_WITH_FINDINGS (exit code 2)."
