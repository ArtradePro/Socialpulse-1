...
# GATE SP-8C-7A: REVISION R18 HOST SYNCHRONIZATION & PREFLIGHT EXECUTION WRAPPER
...
echo "========================================================================"
echo ">>> HIGIENE (PTY) LTD — GATE SP-8C-7A REVISION R18 HOST WRAPPER"
...
if [ "$(id -u)" -ne 0 ]; then
    echo "CRITICAL ERROR: run_sp8c7a_r18.sh must be executed as root (EUID 0)!" >&2
    exit 1
fi
...
# Hardcoded trust anchor for sp8c7a_preflight.sh (Revision R18)
EXPECTED_SCRIPT_SHA256="c086f00b8c52e668d65789b50c7d74e1ba64d9fe60775d15dbe355c8ce11a163"
EXPECTED_SCRIPT_BYTES=58619
EXPECTED_SCRIPT_LINES=1399

# Hardcoded trust anchor for sp8c7a_preflight.sh.sha256 (Revision R18)
EXPECTED_SIDECAR_SHA256="6083fd273928459b137d6b251f2af081d80201258cf9f26095be2f328148f176"
EXPECTED_SIDECAR_BYTES=86
EXPECTED_SIDECAR_LINES=1

# Hardcoded trust anchor for sp8c7a_preflight_runbook.md (Revision R18)
EXPECTED_RUNBOOK_SHA256="2e018173832c88b646ee00f41116e3b9bd681a6534c19927e26797a51ee0b4a3"
EXPECTED_RUNBOOK_BYTES=11064
EXPECTED_RUNBOOK_LINES=109

# Hardcoded trust anchor for sp8c7a_manifest.json (Revision R18)
EXPECTED_MANIFEST_SHA256="aab723fe8aeee311fd86ca31cca0fefb2926cfd891cb68358b1524ff442c4960"
EXPECTED_MANIFEST_BYTES=5060
EXPECTED_MANIFEST_LINES=106
...
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh" 58619 "c086f00b8c52e668d65789b50c7d74e1ba64d9fe60775d15dbe355c8ce11a163"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight.sh.sha256" 86 "6083fd273928459b137d6b251f2af081d80201258cf9f26095be2f328148f176"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_preflight_runbook.md" 11064 "2e018173832c88b646ee00f41116e3b9bd681a6534c19927e26797a51ee0b4a3"
verify_staged_artifact "${STAGE_DIR}/sp8c7a_manifest.json" 5060 "aab723fe8aeee311fd86ca31cca0fefb2926cfd891cb68358b1524ff442c4960"
...
assert rev == "R18", f"revision expected R18, got {rev}"
...
expected_workload_members = {
    "sp8c7a_preflight.sh": {
        "bytes": 58619,
        "lines": 1399,
        "sha256": "c086f00b8c52e668d65789b50c7d74e1ba64d9fe60775d15dbe355c8ce11a163",
        "role": "unified_self_contained_strictly_read_only_host_preflight_script"
    },
    "sp8c7a_preflight.sh.sha256": {
        "bytes": 86,
        "lines": 1,
        "sha256": "6083fd273928459b137d6b251f2af081d80201258cf9f26095be2f328148f176",
        "role": "corroborating_script_checksum_sidecar"
    },
    "sp8c7a_preflight_runbook.md": {
        "bytes": 11064,
        "lines": 109,
        "sha256": "2e018173832c88b646ee00f41116e3b9bd681a6534c19927e26797a51ee0b4a3",
        "role": "operational_preflight_runbook_and_invariants_revision_r18"
    }
}
...
echo "Staged manifest verified: schema 1.0, gate SP-8C-7A, revision R18, exactly 3 payload members with exact field values."

echo ""
echo "========================================================================"
echo ">>> [4/6] Destination Inspection, Atomic Update & Transactional Installation"
echo "========================================================================"
...
check_and_install_artifact() {
    ...
    if [ -e "${dest_file}" ]; then
        ...
        echo "Destination ${fname} exists with differing content; updating atomically to approved artifact."
    fi
...
check_and_install_artifact "sp8c7a_preflight.sh" 58619 "c086f00b8c52e668d65789b50c7d74e1ba64d9fe60775d15dbe355c8ce11a163" "700"
check_and_install_artifact "sp8c7a_preflight.sh.sha256" 86 "6083fd273928459b137d6b251f2af081d80201258cf9f26095be2f328148f176" "600"
check_and_install_artifact "sp8c7a_preflight_runbook.md" 11064 "2e018173832c88b646ee00f41116e3b9bd681a6534c19927e26797a51ee0b4a3" "600"
check_and_install_artifact "sp8c7a_manifest.json" 5060 "aab723fe8aeee311fd86ca31cca0fefb2926cfd891cb68358b1524ff442c4960" "600"
...
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh" 58619 "c086f00b8c52e668d65789b50c7d74e1ba64d9fe60775d15dbe355c8ce11a163" "1001:1001" "700"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight.sh.sha256" 86 "6083fd273928459b137d6b251f2af081d80201258cf9f26095be2f328148f176" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_preflight_runbook.md" 11064 "2e018173832c88b646ee00f41116e3b9bd681a6534c19927e26797a51ee0b4a3" "1001:1001" "600"
reverify_destination_file "${DEST_DIR}/sp8c7a_manifest.json" 5060 "aab723fe8aeee311fd86ca31cca0fefb2926cfd891cb68358b1524ff442c4960" "1001:1001" "600"
...
set +e
EXPECTED_SP8C7A_SHA256="c086f00b8c52e668d65789b50c7d74e1ba64d9fe60775d15dbe355c8ce11a163" \
EXPECTED_SP8C7A_BYTES="58619" \
...
if [ "${PREFLIGHT_STATUS}" -eq 0 ]; then
    ...
    echo "Gate SP-8C-7A Revision R18 host synchronization and preflight execution PASSED with PREFLIGHT_COMPLETE_NO_FINDINGS (exit code 0)."
    exit 0
    ...
    echo "Gate SP-8C-7A Revision R18 host synchronization and preflight execution CONCLUDED with PREFLIGHT_COMPLETE_WITH_FINDINGS (exit code 2)."
